...
def get_params():
    ...
    parser.add_argument("--feedback_agent_cfg", default="./config/feedback_agent.yaml",
                        help="Optional observe-only feedback monitor configuration")
    ...
    # Feedback agent P1: observation only.
    parser.add_argument("--agent_enabled", default=False, action="store_true",
    ...
    # Existing RDP experiment YAMLs remain unchanged; feedback settings live
    # separately and default to disabled.
    if params.feedback_agent_cfg:
        with open(params.feedback_agent_cfg) as f:
            feedback_config = yaml.safe_load(f) or {}
            for k, v in feedback_config.items():
                params.__setattr__(k, v)
