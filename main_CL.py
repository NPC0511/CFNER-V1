...
def main_cl(params):
    ...
    for iteration in range(total_iter): # 遍历每个任务
        ...
        old_entity_list = all_seen_entity_list[:-len(new_entity_list)] if iteration > 0 else []
        feedback_monitor.begin_task(
        ...
        if getattr(params, "action_logging_enabled", False):
            feedback_monitor.record_action(observe_only_policy.validate(ActionRequest(
                action="no_action", step=0, targets=old_entity_list,
                cooldown_steps=observe_only_policy.cooldown_steps,
                trigger_state={
                    "task_id": iteration, "old_types": old_entity_list,
        ...
        for e in range(1, training_epochs+1):
            ...
            for X, y in dataloader_train:
                ...
                # Compute loss
                # batch_loss_rdp changes labels in place to pseudo labels.
                ...
                feedback_monitor.observe_batch(
                    step=step, labels=monitor_labels, logits=trainer.logits,
                ...
                if getattr(params, "adaptive_distillation_enabled", False) and drift:
                    ...
                    if action_record is not None:
                        ...
                        trainer.adaptive_distillation_multiplier = adaptive_policy.multiplier
                ...
                if params.is_train_by_steps: # False
                    ...
                    if not params.debug and step%params.evaluate_interval==0:
                        ...
                        f1_dev_cumul, ma_f1_dev_cumul, f1_dev_each_class_cumul = trainer.evaluate(
                            dataloader_dev_cumul, each_class=True,
        ...
        if params.debug: # False
            ...
            feedback_monitor.end_task({
                "status": "debug_skipped_test", "task_id": iteration,
        ...
        def mean_f1(entity_types):
            values = [f1_test_each_class_cumul[name] for name in entity_types
                      if name in f1_test_each_class_cumul]
            return float(np.mean(values)) if values else None

        monitor_path = feedback_monitor.end_task({
            "status": "completed",
